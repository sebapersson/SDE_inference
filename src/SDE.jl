...
function init_sde_model(calc_alpha, 
    ...
    if typeof(P_mat) <: Array{<:Signed, 1}
        n_col = 1
        n_row = size(P_mat)[1]
        P_mat_use_tmp = Array{Float64, 2}(undef, (n_row, n_col))
        P_mat_use_tmp .= P_mat
    else
        P_mat_use_tmp = convert(Array{Float64, 2}, P_mat)
    end
    dim1, dim2 = size(P_mat_use_tmp)
    P_mat_use = convert(SMatrix{dim_mod, dim2, Float64}, P_mat_use_tmp)
    P_mat_use_transpose = convert(SMatrix{dim2, dim_mod, Float64}, P_mat_use_tmp')
...
@inline function perform_step_em!(u_new, 
    ...
    @views u_new .= (u_old .+ em_obj.alpha_vec .* em_obj.dt + cholesky(em_obj.beta_mat).L*rand_vec .*em_obj.sqrt_dt)
end
